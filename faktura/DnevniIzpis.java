 ...
 * @author radicalb
 */
public class DnevniIzpis extends javax.swing.JFrame implements Printable {

    /**
     * Creates new form DnevniIzpis
     */
    public DnevniIzpis() {
        initComponents();
    }
    ...
    public int print (Graphics graphics, PageFormat pageFormat, int pageIndex) {
        ...
        if(printerTip=="A4"){
            ...
            graphics2D.drawString("IZPIS DOKUMENTOV ZA: "+jComboBox3.getSelectedItem().toString()+"."+(jComboBox1.getSelectedIndex()+1)+" "+jComboBox2.getSelectedItem().toString(), 10, h1+=25);
            Calendar now = GregorianCalendar.getInstance();
            int ura=(now.get(Calendar.AM_PM)==1)?(now.get(Calendar.HOUR)+12):(now.get(Calendar.HOUR));
            ura=ura%24;
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        printJob.setJobName("Dnevni_izpis_"+jComboBox3.getSelectedItem().toString()+"_"+jComboBox1.getSelectedItem().toString()+"_"+jComboBox2.getSelectedItem().toString());
    ...
    private void prikaziRacune(){
        ...
        //model.
            try{
                ...
                s = c.createStatement ();
                String SQLQuery1="select nakupid from nakup where (mesecid_fk="+(jComboBox1.getSelectedIndex()+1)+" AND leto="+jComboBox2.getSelectedItem().toString()+" AND dan="+jComboBox3.getSelectedItem().toString()+");";
                ...
                String SQLQuery3="SELECT DISTINCT(nakup_izdelek.izdelekid_fk) FROM nakup_izdelek INNER JOIN nakup ON nakup.nakupid=nakup_izdelek.nakupid_fk where(mesecid_fk="+(jComboBox1.getSelectedIndex()+1)+" AND leto="+jComboBox2.getSelectedItem().toString()+" AND dan="+jComboBox3.getSelectedItem().toString()+");";
                ...
                for(int i=0;i<jTable2.getRowCount();i++){
                    ...
                    java.sql.Statement s2 = c.createStatement ();
                    String SQLQuery2="select izdelekid_fk, izdelek.naziv,SUM((nakup_izdelek.kolicina*cena.vrednost)-((nakup_izdelek.kolicina*cena.vrednost)*popust/100)) as vrednostbrezdavka ,SUM(nakup_izdelek.kolicina) as  kolicinaskupaj, davek.stopnja, SUM((nakup_izdelek.kolicina*((cena.vrednost+(cena.vrednost*davek.stopnja/100))))-((nakup_izdelek.kolicina*((cena.vrednost+(cena.vrednost*davek.stopnja/100))))*popust/100)) as vrednostspopustom from nakup_izdelek INNER JOIN nakup ON nakup.nakupid=nakup_izdelek.nakupid_fk INNER JOIN izdelek ON izdelek.izdelekid=nakup_izdelek.izdelekid_fk  INNER JOIN cena ON nakup_izdelek.cenaid_fk=cena.cenaid INNER JOIN davek ON nakup_izdelek.davekid_fk=davek.davekid where (izdelekid_fk='"+model2.getValueAt(i, 0) +"' AND mesecid_fk="+(jComboBox1.getSelectedIndex()+1)+" AND leto="+jComboBox2.getSelectedItem().toString()+" AND dan="+jComboBox3.getSelectedItem().toString()+") GROUP BY izdelekid_fk,stopnja, izdelek.naziv;";
                    System.out.println(SQLQuery2);
                    java.sql.ResultSet rs2 = s2.executeQuery (SQLQuery2);
    ...
    private void naloziDni(){
        Calendar now= new GregorianCalendar().getInstance();
        now.set(Integer.parseInt(jComboBox2.getSelectedItem().toString()), jComboBox1.getSelectedIndex(), 1);
        int st_dni=now.getActualMaximum(Calendar.DAY_OF_MONTH);
        if(jComboBox3.getItemCount()!=st_dni){
            jComboBox3.removeAllItems();
            for(int i=1;i<=st_dni;i++){
